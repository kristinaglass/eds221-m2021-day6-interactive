...
```{r}
##select is columns, filter is for rows
...
wb_indicators_tidy <- wb_indicators_clean %>% 
  ...
  dplyr::rename(country = "Country Name",
                acces_fuels_pp = "Access to clean fuels and technologies for cooking (% of population)",
...
```{r}
ggplot(data = wb_indicators_tidy, aes(x = year, y = co2_emissions_kt)) + geom_line(aes(color = country)) +
